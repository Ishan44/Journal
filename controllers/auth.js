...
exports.entryJournal = async (req,res)=>{ //here register is being submitted to authcotroller.register in auth.js
    ...
    const {journalTitle, journalContent } = req.body
    ...
    if(req.cookies.cookieName){
        try{
            ...
            //2. Check if the user still exist
            db.query("select * from users where id = ?",[decoded.id],(err,result)=>{
                uid = result[0] //create a variable, this is passed into pages.js
                ...
                db.query("INSERT INTO journal SET ?",{u_id: uid.id, date: date, content: journalContent, title: journalTitle}, (err, results) => {
                    if(err){
                        console.log(err)
                    }else{
                        console.log("results");
                ...
                db.query("select * from journal where u_id = ?",[uid.id],(err,result)=>{
                    if(err){
                        console.log(err);
                    }else{
                        // console.log("Date result");
                        // console.log(result[1].content);
                        // dbDat=result.map((x)=>x)  
                        // console.log("dbDat"); 
                        // console.log(dbDat);
                        // console.log(dbDat.length);
         ...
         //If no cookie call this function, to make sure that you render the page. Vid 009 5 min.
    }
